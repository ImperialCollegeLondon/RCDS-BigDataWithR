...
GOODWIN_BAT <- compustat[conm %in% c("GOODWIN PLC", "BRITISH AMER TOBACCO PLC")]
```

*NOTE*: This method requires exact match. Any thoughts for a better solution?

```{r select GOODWIN and BAT 1}
GOODWIN_BAT <- compustat[conm %like% "GOODWIN|BRITISH AMER"]
```
...
BAT_01W<-BAT_01W[, YearQ:=as.yearqtr(datadate)]
```
...
compustat[is.finite(prccd)][, .(avg_prccd = mean(prccd)), by = exchg]
```
...
compustat[is.finite(prccd)][, .(avg_prccd = mean(prccd)), by = .(exchg, iid)]
```
...
tail(compustat[is.finite(prccd)][, .(med_prccd = median(prccd)), by = .(gvkey, iid, conm)][order(med_prccd)],15)

# alternative method
head(compustat[is.finite(prccd)][, .(med_prccd = median(prccd)), by = .(gvkey, iid, conm)][order(-med_prccd)],15)
...
```{r daily returns}
compustat <- compustat[is.finite(prccd)][order(gvkey, conm, iid, datadate)][, returns := 
                                                            prccd/shift(prccd, type = "lag") - 1, 
                                                          by = .(gvkey, conm, iid)]
```
...
```{r}
result <- compustat[ (is.finite(prchd) & is.finite(prcld))][, .(high_range = max(prchd) - min(prchd),
                         low_range = max(prcld) - min(prcld)),
                     by = .(gvkey, iid, conm)]
